...
def test_activity_api():
    ...
    try:
        ...
        if "activities" in activities_data:
            ...
            if found_full and found_required:
                ...
                print("❌ Not all created activities were found")
                return False
        else:
            print("❌ Response missing 'activities' field")
            return False
    except Exception as e:
        print(f"❌ Error parsing activities response: {str(e)}")
        return False
    ...
    if response.status_code == 200:
        ...
        if "activities" in activities_data:
            ...
            for activity in activities:
                if activity["id"] == activity_id_full:
                    if activity["date"] == activity_update_data["date"] and \
                       activity["time"] == activity_update_data["time"] and \
                       activity["description"] == activity_update_data["description"]:
                        print("✅ Activity update verified - data matches")
                    else:
                        print("❌ Activity update verification failed - data mismatch")
                        print(f"Expected: {activity_update_data}")
    ...
    if response.status_code == 200:
        ...
        if "activities" in activities_data:
            ...
            for activity in activities:
                if activity["id"] == activity_id_full:
                    print(f"❌ Activity with ID {activity_id_full} was not deleted")
                    return False
    ...
    print("\n" + "=" * 50)
    print("✅ All Activity API tests completed successfully!")
    print("✅ POST /api/admin-registration/{registration_id}/activity - Create new activity: WORKING")
    print("✅ GET /api/admin-registration/{registration_id}/activities - Get all activities: WORKING")
