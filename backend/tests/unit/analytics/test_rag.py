...
from app.core.analytics.schema import ClaudeChatResponse
...
class TestRagServiceChatHistory(IsolatedAsyncioTestCase):
    ...
    async def test_update_chat_appends_message(self):
        ...
        with patch(
            "app.core.analytics.rag.redis_client.get_client",
    ...
    async def test_update_chat_trims_history(self):
        ...
        with patch(
            "app.core.analytics.rag.redis_client.get_client",
            return_value=mock_client,
        ), patch("app.core.analytics.rag.settings.max_chat_length", 20):
    ...
    async def test_update_chat_sets_expiry(self):
        ...
        with patch(
            "app.core.analytics.rag.redis_client.get_client",
            return_value=mock_client,
        ), patch("app.core.analytics.rag.settings.chat_history_ttl", 3600):
    ...
    async def test_get_chat_returns_messages(self):
        ...
        mock_client.lrange = AsyncMock(
            return_value=[json.dumps(msg) for msg in messages]
        )
...
class TestRagServiceToolHandlers(IsolatedAsyncioTestCase):
    ...
    async def test_handle_query_postgres_database_error(self):
        ...
        with patch(
            "app.core.analytics.rag.database.get_connection"
        ) as mock_db:
    ...
    async def test_handle_query_mongodb_valid_pipeline(self):
        ...
        mock_db.__getitem__.return_value = (
            mock_collection  # db["legacy_data"] => mock_collection
        )

        # Patch mongo_client.get_db to return our mock_db
        with patch(
            "app.core.analytics.rag.mongo_client.get_db", return_value=mock_db
        ):
...
class TestRagServicePromptClaude(IsolatedAsyncioTestCase):
    ...
    async def test_prompt_claude_simple_response(self):
        ...
        mock_response.content = [
            MagicMock(
                type="text",
                text="There are 42 patients",
                hasattr=lambda x: x == "text",
            )
        ]
        ...
        with patch(
            "app.core.analytics.rag.settings.anthropic_client", mock_client
        ), patch(
            "app.core.analytics.rag.RagService.get_chat",
            new_callable=AsyncMock,
            return_value=[],
        ), patch(
            "app.core.analytics.rag.RagService.update_chat",
            new_callable=AsyncMock,
        ):
    ...
    async def test_prompt_claude_with_tool_use_postgres(self):
        ...
        with patch(
            ...
            "app.core.analytics.rag.RagService.handle_query_postgres",
            new_callable=AsyncMock,
    ...
    async def test_prompt_claude_with_tool_use_mongodb(self):
        ...
        final_response.content = [
            MagicMock(
                type="text", text="Found data", hasattr=lambda x: x == "text"
            )
        ]
        ...
        with patch(
            ...
            "app.core.analytics.rag.RagService.handle_query_mongodb",
            new_callable=AsyncMock,
    ...
    async def test_prompt_claude_uses_legacy_instruction(self):
        ...
        mock_response.content = [
            MagicMock(
                type="text", text="Result", hasattr=lambda x: x == "text"
            )
        ]
    ...
    async def test_prompt_claude_maintains_history(self):
        ...
        mock_response.content = [
            MagicMock(
                type="text", text="Response", hasattr=lambda x: x == "text"
            )
        ]
