...
class TestLegacyDataSummaryEndpoint(IsolatedAsyncioTestCase):
    ...
    async def test_get_summary_success(self):
        ...
        with patch('app.analytics.services.LegacyDataService.get_legacy_data_summary',
                   new_callable=AsyncMock, return_value=mock_summary) as mock_service:
            
            from app.analytics.router import get_legacy_data_summary
    ...
    async def test_get_summary_service_error(self):
        ...
        with patch('app.analytics.services.LegacyDataService.get_legacy_data_summary',
            ...
            self.assertIn("Failed to get summary data", context.exception.detail)
...
class TestClearLegacyDataEndpoint(IsolatedAsyncioTestCase):
    ...
    async def test_clear_data_success(self):
        ...
        with patch('app.analytics.rag.RagService.clear_chat',
                   new_callable=AsyncMock) as mock_clear_chat, \
             patch('app.analytics.services.LegacyDataService.delete_all_legacy_data',
                   new_callable=AsyncMock, return_value=expected_result) as mock_delete:
            
            from app.analytics.router import clear_legacy_data_summary
    ...
    async def test_clear_data_delete_error(self):
        ...
        with patch('app.analytics.rag.RagService.clear_chat',
            ...
            self.assertEqual(context.exception.status_code, 400)
            self.assertIn("Failed to delete summary data", context.exception.detail)
...
class TestUploadLegacyDataEndpoint(IsolatedAsyncioTestCase):
    ...
    async def test_upload_valid_excel(self):
        ...
        with patch('app.analytics.router.read_legacy_data_file',
                   new_callable=AsyncMock, return_value=df), \
             patch('app.analytics.services.LegacyDataService.upload_legacy_data',
                   new_callable=AsyncMock) as mock_upload, \
             patch('app.analytics.rag.RagService.clear_chat',
                   new_callable=AsyncMock):
            from app.analytics.router import upload_legacy_data
            result = await upload_legacy_data(file=mock_file, user=self.mock_user)
            
            self.assertEqual(result.records_count, 10)
            self.assertIn("Successfully uploaded", result.message)
            self.assertEqual(len(result.preview), 5)  # Should preview 5 records
...
class TestClaudeChatEndpoint(IsolatedAsyncioTestCase):
    ...
    async def test_claude_chat_success(self):
        ...
        with patch('app.analytics.rag.RagService.prompt_claude',
                   new_callable=AsyncMock, return_value=expected_response) as mock_prompt:
            
            from app.analytics.router import claude_chat
    ...
    async def test_claude_chat_legacy_data(self):
        ...
        expected_response = ClaudeChatResponse(
            response="Toronto: 50, Mississauga: 30",
            session_id="test-session"
        )

        with patch('app.analytics.rag.RagService.prompt_claude',
                   new_callable=AsyncMock, return_value=expected_response):
    ...
    async def test_claude_chat_context_error(self):
        ...
        from app.exceptions import ContextRetrievalError
