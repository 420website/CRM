...
from app.core.analytics.tools import QUERY_MONGODB, QUERY_POSTGRES
...
class RagService:
    ...
    async def handle_query_mongodb(user_id: str, pipeline: list) -> str:
        try:
            ...
            if count == 0:
                return json.dumps(
                    {
                        "error": "No legacy data found. Please upload a file first."
                    }
                )
            ...
            results = await collection.aggregate(full_pipeline).to_list(
                length=None
            )
    ...
    async def prompt_claude(
        ...
        data_source_instruction = (
            "Use query_mongo tool for legacy uploaded data."
            if is_legacy
            else "Use query_postgres tool for internal database."
        )
        ...
        try:
            ...
            while response.stop_reason == "tool_use":
                ...
                if tool_use.name == "query_postgres":
                    ...
                    db_results = await RagService.handle_query_mongodb(
                        user_id, query
                    )

                await RagService.update_chat(
                    user_id, "assistant", f"Query: {query}"
                )
