...
async function backupNotesTemplates() {
  ...
  try {
    ...
    console.log(
      `✅ Backed up ${templates.length} Notes templates to ${BACKUP_FILE}`,
    );
  } catch (error) {
...
async function restoreNotesTemplates() {
  ...
  try {
    ...
    if (fs.existsSync(BACKUP_FILE)) {
      try {
        ...
        if (backupTemplates.length > 0) {
          ...
          console.log(
            `📥 Loaded ${backupTemplates.length} Notes templates from backup`,
          );
        }
      } catch (error) {
    ...
    if (templatesToRestore.length > 0) {
      ...
      console.log(
        `✅ Restored ${templatesToRestore.length} Notes templates to database`,
      );
    }
  } catch (error) {
