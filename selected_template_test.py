#!/usr/bin/env python3
"""
Selected Template Field Testing Suite
Tests the newly added selectedTemplate field functionality to ensure template selection persistence is working correctly.
"""
...
class SelectedTemplateFieldTester:
    def __init__(self):
    ...
    def test_1_create_with_default_selected_template(self):
        ...
        # Verify the response includes selectedTemplate
        if 'selectedTemplate' not in result:
            return self.log_test("Create with default selectedTemplate", False, "selectedTemplate not in response")
        
        if result['selectedTemplate'] != 'Select':
            return self.log_test("Create with default selectedTemplate", False, f"Expected 'Select', got '{result['selectedTemplate']}'")
        
        return self.log_test("Create with default selectedTemplate", True, f"Registration ID: {reg_id}")
    ...
    def test_2_create_with_specific_selected_template(self):
        ...
        for template_value in template_values:
            ...
            # Verify the response includes correct selectedTemplate
            if result.get('selectedTemplate') != template_value:
                self.log_test(f"Create with selectedTemplate '{template_value}'", False, 
                            f"Expected '{template_value}', got '{result.get('selectedTemplate')}'")
                continue
            
            self.log_test(f"Create with selectedTemplate '{template_value}'", True, f"Registration ID: {reg_id}")
    ...
    def test_3_create_with_both_templates(self):
        ...
        # Verify both fields are in response
        if result.get('selectedTemplate') != selected_template:
            return self.log_test("Create with both templates", False, 
                               f"selectedTemplate: Expected '{selected_template}', got '{result.get('selectedTemplate')}'")
        
        if result.get('summaryTemplate') != summary_content:
            return self.log_test("Create with both templates", False, 
                               f"summaryTemplate: Expected content not found")
        
        return self.log_test("Create with both templates", True, f"Both fields saved correctly. Registration ID: {reg_id}")
    ...
    def test_5_update_selected_template(self):
        ...
        for new_template in update_templates:
            ...
            success, update_result = self.update_registration(reg_id, update_data)
    ...
    def test_6_valid_template_names(self):
        ...
        for template_name in valid_templates:
            ...
            if result.get('selectedTemplate') != template_name:
                self.log_test(f"Valid template '{template_name}'", False, 
                            f"Expected '{template_name}', got '{result.get('selectedTemplate')}'")
                continue
    ...
    def test_7_backwards_compatibility(self):
        ...
        try:
            ...
            # Verify that selectedTemplate defaults to "Select" even when not provided
            expected_default = "Select"
            if result.get('selectedTemplate') != expected_default:
                return self.log_test("Backwards compatibility", False, 
                                   f"Expected default '{expected_default}', got '{result.get('selectedTemplate')}'")
            
            # Retrieve the record to double-check
            success, get_result = self.get_registration(reg_id)
            if not success:
            ...
            if get_result.get('selectedTemplate') != expected_default:
                return self.log_test("Backwards compatibility", False, 
                                   f"Retrieved record: Expected '{expected_default}', got '{get_result.get('selectedTemplate')}'")
    ...
    def test_8_comprehensive_workflow(self):
        ...
        if result.get('selectedTemplate') != template1 or result.get('summaryTemplate') != summary1:
            return self.log_test("Comprehensive workflow - Verify creation", False, "Initial values not correct")
        ...
        if get_result.get('summaryTemplate') != summary1:
            return self.log_test("Comprehensive workflow - Verify summaryTemplate preserved", False, 
                               "summaryTemplate was not preserved during selectedTemplate update")
        ...
        if final_result.get('selectedTemplate') != template3 or final_result.get('summaryTemplate') != summary3:
            return self.log_test("Comprehensive workflow - Final verification", False, 
                               f"Final values incorrect. selectedTemplate: '{final_result.get('selectedTemplate')}', summaryTemplate length: {len(final_result.get('summaryTemplate', ''))}")
    ...
    def run_all_tests(self):
        ...
        print("🚀 STARTING SELECTED TEMPLATE FIELD TESTING SUITE")
        print("=" * 60)
...
if __name__ == "__main__":
    tester = SelectedTemplateFieldTester()
