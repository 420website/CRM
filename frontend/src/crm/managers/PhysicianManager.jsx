...
export default function PhysicianManager() {
  const { setShowPhysicianManager, genericPhysicians, getPhysicians } =
    useRegistration();
  ...
  const createTemplate = async () => {
    ...
    const result = await GeneralServices.create_general("physician", data);

    if (result.success) {
      getPhysicians();
      setMessage("Created physician successfully.");
    } else {
      if (result.status === 400 || result.status === 409) {
        setError(result.message || "Error creating physician.");
      } else {
        setError("Error creating physician. Please try again.");
      }
    }
    setLoading(false);
  };
  ...
  const updateTemplate = async (id, name, isFrequent) => {
    ...
    const result = await GeneralServices.update_general("physician", id, data);
    ...
    if (result.success) {
      ...
      if (result.status === 400 || result.status === 409) {
        setError(result.message || "Error updating physician.");
      } else {
        setError("Error updating physician. Please try again.");
      }
    }
    setLoading(false);
  };
  ...
  const deleteTemplate = async (id, name) => {
    if (
      !window.confirm(
        `Are you sure you want to delete the "${name}" physician?`,
      )
    ) {
      return;
    }
    ...
    const result = await GeneralServices.delete_general_by_id("physician", id);
    ...
    if (result.success) {
      ...
      if (result.status === 400 || result.status === 409) {
        setError(result.message || "Error deleting physician.");
      } else {
        setError("Error deleting physician. Please try again.");
      }
    }
    setLoading(false);
  };
  ...
  const getFilteredTemplates = () => {
    if (!templatesSearch.trim()) {
      return genericPhysicians;
    }

    const searchTerm = templatesSearch.toLowerCase();
    return genericPhysicians.filter((site) =>
    ...
    <div className="fixed inset-0 bg-black/50 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto mx-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-900">Manage Physicians</h2>
          <button
            type="button"
              ...
              <input
                ...
                onChange={(e) => {
                  const capitalized = e.target.value
                    ...
                    .join(" ");
                  setNewTemplateName(capitalized);
                }}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black"
        ...
        <div>
          ...
          <div className="mb-4">
            ...
            {getFilteredTemplates().filter((s) => s.is_frequent).length ===
              ...
              <p className="text-sm text-gray-500 italic">
                {templatesSearch
                  ? "No frequently used physician match your search."
                  : "No frequently used physician."}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            {getFilteredTemplates().filter((s) => !s.is_frequent).length ===
              ...
              <p className="text-sm text-gray-500 italic">
                {templatesSearch
                  ? "No other physician match your search."
                  : "No other physician."}
              </p>
            )}
          </div>
