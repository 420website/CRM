...
export default function CoverageManager() {
  const { setShowCoverageManager, genericCoverage, getCoverageTypes } =
    useRegistration();
  ...
  const createTemplate = async () => {
    ...
    const result = await GeneralServices.create_general("coverage", data);

    if (result.success) {
      getCoverageTypes();
      setMessage("Created coverage successfully.");
    } else {
      if (result.status === 400 || result.status === 409) {
        setError(result.message || "Error creating coverage.");
      } else {
        setError("Error creating coverage. Please try again.");
      }
    }
    setLoading(false);
  };
  ...
  const updateTemplate = async (id, name, isFrequent) => {
    ...
    const result = await GeneralServices.update_general("coverage", id, data);
    ...
    if (result.success) {
      ...
      if (result.status === 400 || result.status === 409) {
        setError(result.message || "Error updating coverage.");
      } else {
        setError("Error updating coverage. Please try again.");
      }
    }
    setLoading(false);
  };
  ...
  const deleteTemplate = async (id, name) => {
    if (
      !window.confirm(`Are you sure you want to delete the "${name}" coverage?`)
    ) {
      return;
    }
    setLoading(true);
    ...
    const result = await GeneralServices.delete_general_by_id("coverage", id);
    ...
    if (result.success) {
      ...
      if (result.status === 400 || result.status === 409) {
        setError(result.message || "Error deleting coverage.");
      } else {
        setError("Error deleting coverage. Please try again.");
      }
    }
    setLoading(false);
  };
  ...
  const getFilteredTemplates = () => {
    ...
    return genericCoverage.filter((site) =>
    ...
    <div className="fixed inset-0 bg-black/50 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto mx-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-900">Manage Coverages</h2>
          <button
            type="button"
        ...
        <div className="mb-4">
          ...
          <input
            ...
            placeholder="Search by coverage name..."
          />
        </div>
