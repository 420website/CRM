...
class AdminRegistrationComprehensiveTester:
    ...
    def generate_test_data(self):
        ...
        self.test_data = {
            "firstName": f"CompTest{random_suffix}",
            ...
            "email": f"comp.test.{random_suffix}@example.com",
            "language": "English",
            "rnaAvailable": "Yes",
            "rnaSampleDate": "2025-07-01",
            "rnaResult": "Positive",
            "coverageType": "OW"
        }
        ...
        if success:
            ...
            for key, value in self.test_data.items():
                if key in response and response[key] == value:
                    print(f"✅ Field {key} correctly saved as: {value}")
                elif key in response:
                    print(f"❌ Field {key} mismatch - Expected: {value}, Got: {response[key]}")
                else:
    ...
    def test_update_registration(self):
        ...
        if not self.registration_id or not self.test_data:
            print("❌ No registration ID or test data available for testing")
            return False, None
        
        # Create updated data
        updated_data = self.test_data.copy()
        updated_data["specialAttention"] = "Updated special attention notes"
        updated_data["disposition"] = "POCT NEG"  # Change disposition
        updated_data["physician"] = "None"  # Should be set to None for POCT NEG
        ...
        if success:
            print(f"✅ Successfully updated registration with ID: {self.registration_id}")
            # Update our test data to reflect the changes
            self.test_data = updated_data
            return True, response
        else:
            print(f"❌ Failed to update registration with ID: {self.registration_id}")
    ...
    def test_get_pending_registrations(self):
        ...
        if success:
            print(f"✅ Retrieved {len(response)} pending registrations")
            ...
            for reg in response:
                if reg.get('id') == self.registration_id:
                    found = True
                    print(f"✅ Found our test registration in pending list: {reg.get('firstName')} {reg.get('lastName')}")
                    break
            
            if not found and self.registration_id:
                print(f"❌ Our test registration (ID: {self.registration_id}) not found in pending list")
                
            return True, response
        else:
            print("❌ Failed to retrieve pending registrations")
            return False, None
    ...
    def test_pending_after_delete(self):
        """Verify the registration is no longer in the pending list"""
        success, response = self.run_test(
            "Get Pending Admin Registrations After Delete",
            "GET",
            "api/admin-registrations-pending",
            200
        )
        ...
        if success:
            # Check that our registration is NOT in the list
            for reg in response:
                if reg.get('id') == self.registration_id:
                    print(f"❌ Registration still found in pending list after deletion: {reg.get('firstName')} {reg.get('lastName')}")
                    return False, response
            
            print(f"✅ Confirmed registration no longer appears in pending list")
            return True, response
        else:
            print("❌ Failed to retrieve pending registrations")
            return False, None
    ...
    def run_all_tests(self):
        """Run all admin registration tests including delete functionality"""
        print("🚀 Starting Comprehensive Admin Registration Tests")
        ...
        if not create_success:
            print("❌ Cannot proceed with tests - failed to create test registration")
            return False
        ...
        # Get all pending registrations
        self.test_get_pending_registrations()
        
        # Update the registration
        self.test_update_registration()
        
        # Get the updated registration
        self.test_get_registration()
        
        # Delete the registration
        self.test_delete_registration()
        
        # Verify the registration no longer exists
        self.test_get_after_delete()
        
        # Verify the registration is no longer in the pending list
        self.test_pending_after_delete()
...
def main():
    tester = AdminRegistrationComprehensiveTester(backend_url)
