...
def test_complete_admin_registration_workflow():
    """Test the complete admin registration workflow with RNA fields"""
    print("\n" + "=" * 50)
    ...
    print("\n🔍 Step 1: Creating admin registration...")
    ...
    print("\n🔍 Step 2: Getting pending admin registrations...")
    ...
    if response.status_code != 200:
        print(f"❌ Failed to get pending admin registrations - Status: {response.status_code}")
        try:
            print(f"Response: {response.json()}")
        except:
            print(f"Response: {response.text}")
    ...
    print(f"✅ Pending admin registrations retrieved - Status: {response.status_code}")
    ...
    print("\n🔍 Step 3: Getting admin registration by ID...")
    response = requests.get(
    ...
    print("\n🔍 Step 4: Updating admin registration with new RNA values...")
    ...
    # Step 5: Get updated admin registration
    print("\n🔍 Step 5: Getting updated admin registration...")
    response = requests.get(
        f"{backend_url}/api/admin-registration/{registration_id}",
        headers={'Content-Type': 'application/json'}
    )
    ...
    # Step 6: Finalize admin registration
    print("\n🔍 Step 6: Finalizing admin registration...")
    response = requests.post(
    ...
    # Step 7: Verify registration is not in pending list
    print("\n🔍 Step 7: Verifying registration is not in pending list...")
    response = requests.get(
        f"{backend_url}/api/admin-registrations-pending",
        headers={'Content-Type': 'application/json'}
    )
    ...
    print("\n" + "=" * 50)
    print("✅ Complete Admin Registration Workflow Test Passed Successfully")
    print("=" * 50)
...
def test_duplicate_prevention():
    ...
    print("\n" + "=" * 50)
    print("✅ Duplicate Prevention Test Passed Successfully")
    print("=" * 50)
    
    return True

def test_rna_fields_default_values():
    """Test admin registration with default RNA field values"""
    print("\n" + "=" * 50)
    print("🔍 Testing RNA Fields Default Values")
    print("=" * 50)
    
    # Create test data without RNA fields
    test_data = generate_test_data()
    ...
    # Step 1: Create registration without RNA fields
    print("\n🔍 Step 1: Creating registration without RNA fields...")
    response = requests.post(
        f"{backend_url}/api/admin-register",
        json=test_data,
        headers={'Content-Type': 'application/json'}
    ...
    if response.status_code != 200:
        print(f"❌ Failed to create registration - Status: {response.status_code}")
        try:
            print(f"Response: {response.json()}")
        except:
            print(f"Response: {response.text}")
    ...
    print(f"✅ Registration created - Status: {response.status_code}")
    response_data = response.json()
    registration_id = response_data.get('registration_id')
    print(f"Registration ID: {registration_id}")
    
    # Connect to MongoDB to verify default RNA fields
    client = MongoClient(mongo_url)
    db = client[db_name]
    collection = db["admin_registrations"]
    ...
    # Check default RNA fields in MongoDB
    print("\n🔍 Checking default RNA fields in MongoDB...")
    rna_available = registration.get("rnaAvailable")
    rna_sample_date = registration.get("rnaSampleDate")
    rna_result = registration.get("rnaResult")
    ...
    if rna_available != "No":
        print(f"❌ Default rnaAvailable in MongoDB does not match expected 'No'")
        return False
    
    if rna_sample_date is not None:
        print(f"❌ Default rnaSampleDate in MongoDB is not None as expected")
        return False
    
    if rna_result != "Positive":
        print(f"❌ Default rnaResult in MongoDB does not match expected 'Positive'")
        return False
    
    print("✅ Default RNA fields verified in MongoDB")
    
    print("\n" + "=" * 50)
    print("✅ RNA Fields Default Values Test Passed Successfully")
    print("=" * 50)
    
    return True

def main():
    print("🚀 Starting Final Comprehensive Backend Tests for my420.ca")
    ...
    # Test RNA fields default values
    rna_defaults_success = test_rna_fields_default_values()
    
    # Test complete admin registration workflow
    workflow_success = test_complete_admin_registration_workflow()
    
    # Test duplicate prevention
    duplicate_success = test_duplicate_prevention()
    
    # Print overall results
    print("\n" + "=" * 50)
    print("📊 Final Test Results:")
    print(f"API Health Check: {'✅ Passed' if api_health_success else '❌ Failed'}")
    print(f"RNA Fields Default Values: {'✅ Passed' if rna_defaults_success else '❌ Failed'}")
    print(f"Complete Admin Registration Workflow: {'✅ Passed' if workflow_success else '❌ Failed'}")
    print(f"Duplicate Prevention: {'✅ Passed' if duplicate_success else '❌ Failed'}")
    print("=" * 50)
    
    all_passed = api_health_success and rna_defaults_success and workflow_success and duplicate_success
